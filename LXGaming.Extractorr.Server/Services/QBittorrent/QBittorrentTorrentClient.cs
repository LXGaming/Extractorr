...
public class QBittorrentTorrentClient : TorrentClientBase {
    ...
    public async Task AddTorrentTagsAsync(IEnumerable<string> hashes, IEnumerable<string> tags) {
        using var response = await SendAsync(() => {
            var request = new HttpRequestMessage(HttpMethod.Post, "api/v2/torrents/addTags");
            request.Content = new FormUrlEncodedContent(new Dictionary<string, string?> {
    ...
    public async Task SetTorrentTagsAsync(IEnumerable<string> hashes, IEnumerable<string> tags) {
        using var response = await SendAsync(() => {
            var request = new HttpRequestMessage(HttpMethod.Post, "api/v2/torrents/setTags");
            request.Content = new FormUrlEncodedContent(new Dictionary<string, string?> {
                { "hashes", string.Join('|', hashes) },
                { "tags", string.Join(',', tags) }
            });
    ...
    public async Task RemoveTorrentTagsAsync(IEnumerable<string> hashes, IEnumerable<string> tags) {
        using var response = await SendAsync(() => {
            var request = new HttpRequestMessage(HttpMethod.Post, "api/v2/torrents/removeTags");
            request.Content = new FormUrlEncodedContent(new Dictionary<string, string?> {
                { "hashes", string.Join('|', hashes) },
                { "tags", string.Join(',', tags) }
            });
